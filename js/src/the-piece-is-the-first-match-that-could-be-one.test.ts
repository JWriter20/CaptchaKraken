...
const el = (opts: { width?: number, visible?: boolean, box?: Box, throws?: boolean }): Omit<FakeNode, 'matches'> => ({
  visible: opts.visible,
  throws: opts.throws,
  box: opts.box !== undefined ? opts.box : { x: 10, y: 20, width: opts.width ?? 61, height: 160 },
});

const scope = (mapping: Record<string, Array<Omit<FakeNode, 'matches'>>>, unparsable: string[] = []) =>
  fakeDom(Object.entries(mapping).flatMap(([selector, nodes]) => nodes.map((n) => ({ ...n, matches: [selector] }))), { unparsable });
...
test('a generic selector matching the container first still finds the piece', async () => {
  const box = await measure(scope({ '[class*="jigsaw"]': [el({ width: 320 }), el({ width: 61 })] }));
...
test('a selector this adapter cannot parse does not end the search', async () => {
  const box = await measure(scope({ '.yidun_jigsaw': [el({ width: 61 })] }, ['.geetest_slice', '.tencent-captcha-dy__fg-item']));
  assert.equal(box?.width, 61, 'an unparsable selector stopped the list');
});

test('an invisible match is not the piece', async () => {
  const box = await measure(scope({ '.geetest_slice': [el({ width: 80, visible: false })], '.yidun_jigsaw': [el({ width: 61 })] }));
  assert.equal(box?.width, 61);
});

test('a match the page will not give a box for is not the piece', async () => {
  const box = await measure(scope({ '.geetest_slice': [el({ box: null }), el({ width: 0 })], '.yidun_jigsaw': [el({ width: 61 })] }));
...
test('a candidate that vanishes mid-read is not the piece', async () => {
  const box = await measure(scope({ '.geetest_slice': [el({ throws: true })], '.yidun_jigsaw': [el({ width: 61 })] }));
...
test('the first selector that yields a plausible piece wins', async () => {
  const box = await measure(scope({ '.geetest_slice': [el({ width: 80 })], '[class*="jigsaw"]': [el({ width: 61 })] }));
